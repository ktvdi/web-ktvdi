...
<head>
    ...
    <link rel="icon" href="{{ url_for('static', filename='images/ktvdi.png') }}" type="image/x-icon" />
...
<body class="{% block body_class %}default-page{% endblock %}">
    <header>
        ...
        <div class="logo">
          <img src="{{ url_for('static', filename='https://iili.io/fztYx8F.md.png') }}" alt="Logo KTVDI">
        </div>
        <nav>
            <ul>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
          ...
          // Menggunakan URL untuk memeriksa apakah ini halaman utama
          const isHomePage = window.location.pathname === '/';  // Memeriksa apakah URL adalah halaman utama
    
          if (footerss) {
            if (isHomePage) {
              // Animasi typewriter untuk headline jika halaman utama
              typeWriter(headline, headline.textContent, 0, () => {
                ...
                // Menampilkan footer setelah animasi selesai
                footerss.style.display = 'block';
                footerss.classList.add('fade-in-up');
              });
            } else {
              // Di halaman lain, footer langsung tampil
              footerss.style.display = 'block';
            }
          }
        });
