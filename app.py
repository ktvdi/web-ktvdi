...
model = genai.GenerativeModel("gemini-2.5-flash", system_instruction="Anda adalah Asisten KTVDI. Jawab singkat dan jelas.")
...
@app.route("/")
def home():
    # --- LOGIKA MANUAL AGAR INDEX.HTML LAMA TIDAK ERROR ---
                    ...
                    if isinstance(wilayah_data, dict): 
                        ...
                        for penyelenggara, details in wilayah_data.items():
                            if 'siaran' in details:
                                ...
                                for siaran in details['siaran']:
                                    siaran_counts[siaran.lower()] += 1
                            
                            if 'last_updated_date' in details:
                                try:
                                    current_updated_time = datetime.strptime(details['last_updated_date'], '%d-%m-%Y')
                                    if last_updated_time is None or current_updated_time > last_updated_time:
                                        last_updated_time = current_updated_time
                                except: pass

    # Menentukan siaran terbanyak
    if siaran_counts:
        most_common = siaran_counts.most_common(1)[0]
    ...
    # Render template dengan variabel LENGKAP
...
# --- ROUTE FITUR BARU (CCTV & JADWAL SHOLAT) ---
@app.route("/cctv")
def cctv_page():
...
def jadwal_sholat_page():
    ...
    daftar_kota = [
        ...
        {"id": "1221", "nama": "Kota Samarinda"},
        {"id": "2701", "nama": "Kota Makassar"},
        {"id": "0612", "nama": "Kota Palembang"},
        {"id": "1222", "nama": "Kota Banda Aceh"},
...
def verify_register():
    ...
    if request.method == "POST":
        if str(ref.child(f'pending_users/{user}/otp').get()) == request.form.get("otp"):
            ...
            ref.child(f'users/{user}').set({"nama": d['nama'], "email": d['email'], "password": d['password'], "points": 0})
...
def get_wilayah():
    d = ref.child(f"siaran/{request.args.get('provinsi')}").get() or {}
    return jsonify({"wilayah": list(d.keys())})

@app.route("/get_mux")
def get_mux():
    d = ref.child(f"siaran/{request.args.get('provinsi')}/{request.args.get('wilayah')}").get() or {}
    return jsonify({"mux": list(d.keys())})

@app.route("/get_siaran")
def get_siaran():
    return jsonify(ref.child(f"siaran/{request.args.get('provinsi')}/{request.args.get('wilayah')}/{request.args.get('mux')}").get() or {})
