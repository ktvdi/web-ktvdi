...
# --- 4. AI GEMINI (UPDATED KEY & MODEL) ---
GEMINI_KEY = "AIzaSyCqEFdnO3N0JBUBuaceTQLejepyDlK_eGU"

try:
    genai.configure(api_key=GEMINI_KEY)
    # Menggunakan model 2.0 Flash sesuai request
...
Tugas: Menjawab pertanyaan seputar TV Digital, STB, dan Gangguan Sinyal.
"""
...
def register():
    if request.method == "POST":
        ...
        if u in users:
            flash("Username ini sudah digunakan.", "error"); return render_template("register.html")
        for uid, data in users.items():
            if normalize_input(data.get('email')) == e:
                flash("Email ini sudah terdaftar.", "error"); return render_template("register.html")
        ...
        try:
            ...
            msg.body = f"""Yth. Bapak/Ibu/Kak {n},
...
def verify_register():
    ...
    if request.method == "POST":
        ...
        if not p or time.time() > p.get('expiry', 0):
            flash("Kode OTP telah kedaluwarsa. Silakan daftar ulang.", "error")
            if p: ref.child(f'pending_users/{u}').delete()
...
Kami sangat bangga menyambut Anda sebagai bagian dari keluarga besar Komunitas TV Digital Indonesia. Mari bersama-sama kita wujudkan akses informasi penyiaran yang merata, jujur, dan bermanfaat bagi seluruh masyarakat.
    ...
    if 'user' in session and not session.get('religi_notif_sent'):
        try:
            ...
            if user_data and user_data.get('email'):
                ...
                msg = Message("🕌 Pengingat Ibadah & Kebaikan", recipients=[user_data['email']])
...
Kami mendoakan semoga hari Kakak senantiasa diberkahi kesehatan, kelancaran rezeki yang halal, dan ketenangan hati. Tetaplah menjadi pribadi yang jujur dan inspiratif bagi sekitar.
...
def trigger_daily_blast():
    try:
        ...
        PANDUAN PENULISAN:
        1. Tone: Profesional, Elegan, namun Sangat Peduli (Empatik). Seperti surat dari CEO/Pimpinan kepada anggota tim yang dihargai.
        2. Struktur:
           - Pembuka: Sapaan hormat kepada [NAMA_USER], ucapan terima kasih atas dedikasi mereka.
           - Inti: Rangkuman berita di atas secara naratif (bukan poin kaku).
...
def chatbot_api():
    ...
    if not model: return jsonify({"response": "Sistem AI sedang dalam pemeliharaan berkala."})
    try:
        # Prompt untuk jawaban profesional
        full_prompt = f"""
        Anda adalah MODI, Customer Service Virtual KTVDI.
        Gaya Bicara: Profesional, Sopan, Baku, Informatif, dan Sangat Membantu (Service Excellence).
        
        Pertanyaan Pengguna: {data.get('prompt')}
        
        Jawaban (Berikan solusi konkret dan akhiri dengan tawaran bantuan lain):
        """
